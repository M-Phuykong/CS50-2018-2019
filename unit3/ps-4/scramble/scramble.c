...
void draw(void)
{
    ...
    for (int row = 0; row < DIMENSION; row++)
    {
        for (int col = 0; col < DIMENSION; col++)
        {
            //Print the character of each grid that is generated randomly
...
bool lookup(string s)
{
    //Assigning variable for binary search
    int n = dictionary.size;
    int start = 0;
    int end = dictionary.size - 1;

    //Binary Search through the list of dictionary word (list is already sorted)
    while (n > 0)
    {
        int m = (start + end) / 2;

        //Use strcmp() to compare string; return 0 if the same
        if (strcmp(s, dictionary.words[m].letters) == 0)
        {
            //A condition to check if the word has already been flagged or not
            if (dictionary.words[m].found == 1)
            {
                //Return False if the user already used the word once
                return false;
            }
            //If its a new word then flagged the word and return true
            else
            {
                dictionary.words[m].found = true;
                return true;
            }
        }
        //Check left of the list if s is smaller than the middle word
        else if (strcmp(s, dictionary.words[m].letters) < 0)
        {
            end = m - 1;
        }
        //Check right of the list if the middle word is smaller than 's'
        else if (strcmp(s, dictionary.words[m].letters) > 0)
        {
            start = m + 1;
        }
...
void scramble(void)
{
    //Declare another array of the board that retains the original board
    char tempgrid[DIMENSION][DIMENSION];
    //Declare a variable for accessing the board's index
    int m;

    //Populating the new array so that it has the original value before rotating
    for (int row = 0; row < DIMENSION; row++)
    {
    ...
    //Loop through grid[][] and change it based on the new board
    for (int row = 0; row < DIMENSION; row++)
    {
        //Hard code m = 3 since the maximum DIMENSION is 4
        m = 3;

        for (int col = 0; col < DIMENSION; col++)
        {
            //Change the value of the grid board
            // Ex: grid[0][0] becomes grid[3][0] (3 is the value of m)
            grid[row][col] = tempgrid[m][row];
            //Decrease m one at a time until the "col" loops end
            m--;
        }
    }
}