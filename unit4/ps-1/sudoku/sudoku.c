...
int main(int argc, char *argv[])
{
    ...
    //Populate the start board for comparison
    for (int y = 0; y < 9; y++)
    {
        for (int x = 0; x < 9; x++)
        {
            //take the same value as board
            g.startboard[y][x] = g.board[y][x];
        }
    }
    ...
    do
    {
        ...
        switch (ch)
        {
            ...
            //Let the user move the cursor up
            case KEY_UP:

                //If statement to wrap back around so the user can't go out of board
                if (g.y <= 0)
                {
                    g.y += 9 ;
                }
